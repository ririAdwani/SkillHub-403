...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_category_name'])) {
    ...
    if ($newCat === '') {
        $categoryMsg = 'Category name cannot be empty.'; $categoryType = 'error';
    } else {
        try {
            $pdo->prepare("INSERT INTO categories (category_name) VALUES (:name)")->execute([':name' => $newCat]);
            $categoryMsg = 'Category "' . h($newCat) . '" added successfully!'; $categoryType = 'success';
        } catch (PDOException $e) {
            $categoryMsg = 'That category already exists.'; $categoryType = 'error';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_category_id'])) {
    ...
    if ($catId > 0 && $catNewName !== '') {
        try {
            $pdo->prepare("UPDATE categories SET category_name = :name WHERE category_id = :id")->execute([':name' => $catNewName, ':id' => $catId]);
            $categoryMsg = 'Category updated to "' . h($catNewName) . '".'; $categoryType = 'success';
        } catch (PDOException $e) {
            $categoryMsg = 'That category name already exists.'; $categoryType = 'error';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_message') {
    ...
    if ($msgId > 0 && $newText !== '') {
        try {
            $pdo->prepare("UPDATE feedback_messages SET message = :msg WHERE message_id = :id")->execute([':msg' => $newText, ':id' => $msgId]);
            $pdo->prepare("UPDATE feedback SET admin_reply = :msg WHERE feedback_id = :fid")->execute([':msg' => $newText, ':fid' => $feedbackId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'resolve_feedback') {
    ...
    if ($fid > 0) {
        try {
            $pdo->prepare("UPDATE feedback SET resolved = 1 WHERE feedback_id = :id")->execute([':id' => $fid]);
            echo json_encode(['success' => true]);
        } catch (PDOException $e) {
            try {
                ...
                $pdo->prepare("UPDATE feedback SET resolved = 1 WHERE feedback_id = :id")->execute([':id' => $fid]);
                echo json_encode(['success' => true]);
            } catch (PDOException $e2) { echo json_encode(['success' => false, 'message' => 'Could not resolve feedback.']); }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reply_feedback') {
    ...
    if ($fid > 0 && $reply !== '') {
        try {
            try { $pdo->prepare("INSERT INTO feedback_messages (feedback_id, sender, message) VALUES (:fid, 'admin', :msg)")->execute([':fid' => $fid, ':msg' => $reply]); } catch (PDOException $e2) {}
            $pdo->prepare("UPDATE feedback SET admin_reply = :reply WHERE feedback_id = :id")->execute([':reply' => $reply, ':id' => $fid]);
    ...
    } else { echo json_encode(['success' => false, 'message' => 'Reply cannot be empty.']); }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_instructor_id'])) {
    ...
    if ($eid > 0 && $eName !== '') {
        try {
            $pdo->prepare("UPDATE instructors SET full_name=:name, title=:title, email=:email, specialty=:specialty, experience=:experience WHERE instructor_id=:id")
                ->execute([':name'=>$eName,':title'=>$eTitle?:null,':email'=>$eEmail?:null,':specialty'=>$eSpecialty?:null,':experience'=>$eExp?:null,':id'=>$eid]);
            $instructorMsg = 'Instructor updated successfully.'; $instructorType = 'success';
        } catch (PDOException $e) { $instructorMsg = 'Could not update instructor.'; $instructorType = 'error'; }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_instructor_name'])) {
    ...
    if ($newName === '') {
        $instructorMsg = 'Instructor name is required.'; $instructorType = 'error';
    } else {
        try {
            $pdo->prepare("INSERT INTO instructors (full_name, title, email, specialty, experience) VALUES (:name, :title, :email, :specialty, :experience)")
                ->execute([':name'=>$newName,':title'=>$newTitle?:null,':email'=>$newEmail?:null,':specialty'=>$newSpecialty?:null,':experience'=>$newExp?:null]);
            $instructorMsg = 'Instructor "' . h($newTitle ? $newTitle . ' ' . $newName : $newName) . '" added!';
            $instructorType = 'success';
        } catch (PDOException $e) { $instructorMsg = 'Could not add instructor.'; $instructorType = 'error'; }
    }
}
...
switch ($feedbackFilter) {
    case '7days':  $feedbackSQL .= " AND submitted_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)"; break;
    case '30days': $feedbackSQL .= " AND submitted_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)"; break;
    case 'specific':
...
try { $instructors = $pdo->query("SELECT * FROM instructors ORDER BY full_name ASC")->fetchAll(); }
...
try {
    ...
    $workshops = $pdo->query("SELECT w.*, c.category_name, '' AS instructor_name, NULL AS instr_id FROM workshops w JOIN categories c ON w.category_id = c.category_id ORDER BY w.workshop_date ASC")->fetchAll();
    $instructors = [];
}
...
try {
    ...
    } catch (PDOException $e2) { $feedbackMessages = []; }
} catch (PDOException $e) {
    $feedbackList = []; $feedbackMessages = []; $feedbackTableMissing = true;
}
...
// Today's date for the date min attribute — prevents selecting past dates
...
<head>
  ...
  <style>
    /* ── IMAGE INPUT ── */
    .img-url-input { width:100%; padding:10px 14px; border:1.5px solid #e2e8f0; border-radius:10px; font-size:0.9rem; font-family:inherit; color:#0f172a; background:#f8fafc; outline:none; transition:border-color 0.15s,box-shadow 0.15s; box-sizing:border-box; }
    .img-url-input:focus { border-color:#2c7be5; background:white; box-shadow:0 0 0 3px rgba(44,123,229,0.08); }
    .img-help-text { color:#94a3b8; font-size:0.76rem; margin-top:6px; display:block; }
    .learning-points-hint { font-size:0.76rem; color:#94a3b8; margin-top:5px; display:block; line-height:1.5; }
...
<div class="admin-layout">
  ...
  <aside class="admin-sidebar">
    ...
    <div class="admin-sidebar-user">
      <?php if (!empty($adminProfileImage)): ?>
        <img src="<?= h('../../' . $adminProfileImage) ?>" alt="<?= h($adminFullName) ?>"
          class="admin-sidebar-avatar admin-sidebar-avatar-img"
          onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
        <div class="admin-sidebar-avatar" style="display:none"><?= strtoupper(substr($adminFullName, 0, 1)) ?></div>
      <?php else: ?>
    ...
    <nav class="admin-sidebar-nav">
      ...
      <a href="#" class="admin-sidebar-link active" id="sidebar-link-workshops" onclick="sidebarNav('top', this); return false;">
        <i class="fa-solid fa-gauge"></i><span>Workshop Manager</span>
      </a>
      <a href="#instructors" class="admin-sidebar-link" id="sidebar-link-instructors" onclick="sidebarNav('instructors', this); return false;">
        <i class="fa-solid fa-chalkboard-user"></i><span>Instructors</span>
      </a>
      <a href="#feedback" class="admin-sidebar-link" id="sidebar-link-feedback" onclick="sidebarNav('feedback', this); return false;">
        <i class="fa-solid fa-comments"></i><span>Student Feedback</span>
        <?php $unrepliedCount = count(array_filter($feedbackList, fn($f) => $f['admin_reply'] === null)); if ($unrepliedCount > 0): ?>
    ...
    <div class="admin-sidebar-footer">
      <a href="../../index.php" class="admin-sidebar-footer-link" target="_blank"><i class="fa-solid fa-globe"></i> View Live Site</a>
      <a href="../../pages/logout.php" class="admin-sidebar-footer-link admin-sidebar-logout"><i class="fa-solid fa-right-from-bracket"></i> Sign Out</a>
    </div>
  </aside>
  ...
  <main class="admin-main">
    ...
    <div class="admin-topbar">
      ...
      <button class="btn btn-primary" id="open-add-modal"><i class="fa-solid fa-plus"></i> Add Workshop</button>
    </div>
    ...
    <div class="admin-stats-row">
      <div class="admin-stat-card">
        ...
        <div><div class="admin-stat-number" id="stat-total"><?= count($workshops) ?></div><div class="admin-stat-label">Workshops</div></div>
      </div>
      <div class="admin-stat-card">
      ...
      <div class="admin-stat-card">
        ...
        <div><div class="admin-stat-number" id="stat-feedback"><?= count($feedbackList) ?></div><div class="admin-stat-label">Feedback</div></div>
      </div>
    </div>
    ...
    <div class="admin-panel" style="margin-bottom:24px;">
      <div class="admin-panel-header">
        <div><h2>All Workshops</h2><p class="admin-panel-sub">Click Edit to update or Delete to remove a workshop.</p></div>
      </div>
      <div class="table-wrapper" style="margin:0;border-radius:0;box-shadow:none;">
        <table id="workshops-table">
          <thead><tr><th>#</th><th>Title</th><th>Category</th><th>Date</th><th>Time</th><th>Seats</th><th>Instructor</th><th>Actions</th></tr></thead>
          <tbody id="workshops-tbody">
            ...
            <tr data-id="<?= $w['workshop_id'] ?>">
              ...
              <td class="admin-instructor-cell"><?php $iName = trim($w['instructor_name'] ?? ''); echo ($iName !== '') ? h($iName) : '<span class="admin-unassigned">Unassigned</span>'; ?></td>
              <td class="admin-action-btns">
                <button class="btn-admin-edit"
                  ...
                  data-hook-message="<?= h($w['hook_message'] ?? '') ?>"
                  data-good-fit-for="<?= h($w['good_fit_for'] ?? '') ?>"
                ...
                <button class="btn-admin-delete" data-id="<?= $w['workshop_id'] ?>"><i class="fa-solid fa-trash"></i> Delete</button>
              </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
    ...
    <div class="admin-panel" style="margin-bottom:24px;" id="categories">
      <div class="admin-panel-header"><div><h2>Workshop Categories</h2><p class="admin-panel-sub">Add new categories or edit existing ones.</p></div></div>
      <div class="admin-category-body">
        <?php if ($categoryMsg): ?>
          <div class="admin-alert admin-alert-<?= $categoryType ?>" id="category-alert">
            <i class="fa-solid fa-<?= $categoryType === 'success' ? 'circle-check' : 'circle-xmark' ?>"></i><?= $categoryMsg ?>
          </div>
          <script>setTimeout(function(){var el=document.getElementById('category-alert');if(el){el.style.transition='opacity 0.5s';el.style.opacity='0';setTimeout(function(){el.remove();},500);}},3000);</script>
        <?php endif; ?>
        <div class="admin-category-list">
          ...
          <div class="admin-category-row" id="cat-row-<?= $cat['category_id'] ?>">
            <div class="admin-category-view" id="cat-view-<?= $cat['category_id'] ?>">
              ...
              <button class="btn-cat-edit" onclick="startEditCategory(<?= $cat['category_id'] ?>, '<?= h($cat['category_name']) ?>')"><i class="fa-solid fa-pen"></i> Edit</button>
            </div>
            <form class="admin-category-edit-form" id="cat-edit-<?= $cat['category_id'] ?>" method="post" action="admin.php#categories" style="display:none;">
        ...
        <div class="admin-add-category-section">
          ...
          <form method="post" action="admin.php#categories" class="admin-add-category-form">
            <input type="text" name="new_category_name" placeholder="e.g. Data Science..." maxlength="100" required />
    ...
    <div class="admin-panel" style="margin-bottom:24px;" id="instructors">
      <div class="admin-panel-header"><div><h2>Instructors</h2><p class="admin-panel-sub">Specialty and experience appear in the workshop details popup on the student site.</p></div></div>
      <div class="admin-category-body">
        <?php if ($instructorMsg): ?>
          <div class="admin-alert admin-alert-<?= $instructorType ?>" id="instructor-alert">
            <i class="fa-solid fa-<?= $instructorType === 'success' ? 'circle-check' : 'circle-xmark' ?>"></i><?= $instructorMsg ?>
          </div>
          <script>setTimeout(function(){var el=document.getElementById('instructor-alert');if(el){el.style.transition='opacity 0.5s';el.style.opacity='0';setTimeout(function(){el.remove();},500);}},3000);</script>
        <?php endif; ?>
          ...
          <?php else: foreach ($instructors as $inst): ?>
            <div class="admin-instructor-chip" id="inst-chip-<?= $inst['instructor_id'] ?>">
              <div class="admin-inst-view" id="inst-view-<?= $inst['instructor_id'] ?>">
                ...
                <button class="btn-cat-edit" onclick="startEditInstructor(<?= $inst['instructor_id'] ?>,'<?= h($inst['title']??'') ?>','<?= h($inst['full_name']) ?>','<?= h($inst['email']??'') ?>','<?= h($inst['specialty']??'') ?>','<?= h($inst['experience']??'') ?>')">
                  <i class="fa-solid fa-pen"></i> Edit
                </button>
              </div>
              <form class="admin-inst-edit-form" id="inst-edit-<?= $inst['instructor_id'] ?>" method="post" action="admin.php#instructors" style="display:none;">
                <input type="hidden" name="edit_instructor_id" value="<?= $inst['instructor_id'] ?>" />
                <input type="text"  name="edit_instructor_title"      placeholder="Title" maxlength="20"  id="inst-edit-title-<?= $inst['instructor_id'] ?>"     style="max-width:90px;" />
                ...
                <input type="text"  name="edit_instructor_specialty"  placeholder="Specialty" maxlength="255" id="inst-edit-specialty-<?= $inst['instructor_id'] ?>" style="min-width:180px;" />
    ...
    <div class="admin-panel" id="feedback">
      <div class="admin-panel-header">
        <div><h2><i class="fa-solid fa-comments" style="color:#f97316;margin-right:8px;"></i>Student Feedback</h2><p class="admin-panel-sub">Reply to students or mark as resolved.</p></div>
      ...
      <?php if ($feedbackTableMissing): ?>
        <div style="padding:24px;"><div class="admin-alert admin-alert-error"><i class="fa-solid fa-triangle-exclamation"></i> Run <strong>feedback_table.sql</strong> in phpMyAdmin to enable this section.</div></div>
      <?php elseif (empty($feedbackList)): ?>
        <div class="admin-empty-state"><div class="admin-empty-icon"><i class="fa-solid fa-inbox"></i></div><h3>No feedback yet</h3><p>Student feedback will appear here.</p></div>
      <?php else: ?>
          ...
          <div class="feedback-card">
            <div class="feedback-card-top">
              <div class="feedback-user">
                ...
                <div class="feedback-user-details"><strong><?= h($fb['name']) ?></strong><span><?= h($fb['email']) ?></span></div>
              </div>
              <div class="feedback-badges">
                <?php $rc = match($fb['rating']) { 'Good'=>'feedback-rating-good','Average'=>'feedback-rating-average','Poor'=>'feedback-rating-poor',default=>'' }; $ri = match($fb['rating']) { 'Good'=>'fa-face-smile','Average'=>'fa-face-meh','Poor'=>'fa-face-frown',default=>'fa-star' }; ?>
                <span class="feedback-rating <?= $rc ?>"><i class="fa-solid <?= $ri ?>"></i> <?= h($fb['rating']) ?></span>
                <span class="feedback-date"><i class="fa-regular fa-clock"></i> <?= h($fb['submitted_formatted']) ?></span>
              </div>
            </div>
            <?php if ($fb['workshops_interested'] || $fb['preferred_time']): ?>
            <div class="feedback-details">
              <?php if ($fb['workshops_interested']): ?><div class="feedback-detail-row"><span class="feedback-detail-label"><i class="fa-solid fa-bookmark"></i> Wants more of</span><span><?= h($fb['workshops_interested']) ?></span></div><?php endif; ?>
              <?php if ($fb['preferred_time']): ?><div class="feedback-detail-row"><span class="feedback-detail-label"><i class="fa-solid fa-clock"></i> Prefers</span><span><?= h($fb['preferred_time']) ?></span></div><?php endif; ?>
            </div>
            <?php endif; ?>
            <?php if ($fb['comments']): ?><div class="feedback-comment"><i class="fa-solid fa-quote-left"></i> <?= h($fb['comments']) ?></div><?php endif; ?>
            <?php $msgs = $feedbackMessages[$fb['feedback_id']] ?? []; if (!empty($msgs)): ?>
              <div class="feedback-message-history" id="msg-history-<?= $fb['feedback_id'] ?>">
                ...
                <div class="feedback-reply-display admin-msg-bubble"
                  ...
                  <div class="feedback-reply-label"><i class="fa-solid fa-reply"></i> Admin · <?= date('M j, g:i A', strtotime($msg['sent_at'])) ?><span class="msg-edit-hint">double-click to edit</span></div>
            ...
            <?php elseif (!empty($fb['admin_reply'])): ?>
              <div class="feedback-reply-display"><div class="feedback-reply-label"><i class="fa-solid fa-reply"></i> Your Reply</div><p><?= h($fb['admin_reply']) ?></p></div>
            <?php endif; ?>
            <div class="feedback-reply-actions" id="reply-actions-<?= $fb['feedback_id'] ?>">
              <button class="btn-reply-open" onclick="openReplyBox(<?= $fb['feedback_id'] ?>, '')"><i class="fa-solid fa-paper-plane"></i> <?= $fb['admin_reply'] ? 'New Message' : 'Write Reply' ?></button>
              <button class="btn-resolve" onclick="resolveFeedback(<?= $fb['feedback_id'] ?>, this)"><i class="fa-solid fa-circle-check"></i> Resolve</button>
            </div>
            <div class="feedback-reply-box" id="reply-box-<?= $fb['feedback_id'] ?>" style="display:none">
              ...
              <div class="feedback-reply-box-actions">
                ...
                <button class="btn-reply-save" onclick="submitReply(<?= $fb['feedback_id'] ?>)"><i class="fa-solid fa-paper-plane"></i> Send Reply</button>
              </div>
            </div>
          </div>
          <?php endforeach; ?>
  ...
  <div class="admin-modal">
    <div class="admin-modal-header">
      <div class="admin-modal-title-group">
        ...
        <div><p class="admin-modal-kicker">Workshop Management</p><h2>Add New Workshop</h2></div>
      </div>
    ...
    <form id="add-workshop-form" novalidate>
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <select id="add-category" name="category_id">
            ...
            <?php foreach ($categories as $cat): ?><option value="<?= $cat['category_id'] ?>"><?= h($cat['category_name']) ?></option><?php endforeach; ?>
          </select>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea id="add-hook-message" name="hook_message" rows="2" maxlength="500"
          placeholder="e.g. Master Python in one afternoon. No prior experience needed."></textarea>
      ...
      <div class="form-row">
        <div class="form-group">
          <!-- min set to today — admin cannot select a past date -->
          <label for="add-date">Workshop Date <span class="required">*</span></label>
          <input type="date" id="add-date" name="workshop_date" min="<?= $todayDate ?>" />
        </div>
        <div class="form-group">
      ...
      <!-- Start time + Duration → auto-calculates end time -->
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <div class="duration-row">
          <div class="duration-field">
            <input type="number" id="add-dur-hours" min="0" max="12" placeholder="0" />
            <span class="duration-unit">hr</span>
          </div>
          <div class="duration-field">
            <input type="number" id="add-dur-mins" min="0" max="59" placeholder="0" />
            <span class="duration-unit">min</span>
          </div>
          <!-- End time shown here automatically and sent to API -->
          <span class="end-time-preview" id="add-end-preview"></span>
        </div>
  ...
  <div class="admin-modal">
    <div class="admin-modal-header">
      <div class="admin-modal-title-group">
        ...
        <div><p class="admin-modal-kicker">Workshop Management</p><h2>Edit Workshop</h2></div>
      </div>
    ...
    <form id="edit-workshop-form" novalidate>
      ...
      <!-- Hook Message -->
      <div class="form-group">
        <label for="edit-hook-message">Hook Message <span style="color:#94a3b8;font-weight:400;">(optional — max 2 sentences)</span></label>
        <textarea id="edit-hook-message" name="hook_message" rows="2" maxlength="500"
          placeholder="e.g. Master Python in one afternoon. No prior experience needed."></textarea>
        <span class="learning-points-hint"><i class="fa-solid fa-bolt" style="margin-right:3px;color:#f59e0b;"></i> Shown on the workshop card.</span>
      </div>

      <!-- Good Fit For -->
      <div class="form-group">
        <label for="edit-good-fit-for">Good Fit For <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
        <textarea id="edit-good-fit-for" name="good_fit_for" rows="3"
          placeholder="e.g. Students interested in data careers."></textarea>
        <span class="learning-points-hint"><i class="fa-solid fa-users" style="margin-right:3px;color:#8b5cf6;"></i> Shown in the workshop details modal.</span>
      </div>

      <div class="form-row">
        <div class="form-group">
          <label for="edit-date">Workshop Date <span class="required">*</span></label>
          <input type="date" id="edit-date" name="workshop_date" min="<?= $todayDate ?>" />
        </div>
        <div class="form-group">
      ...
      <div class="form-group">
        ...
        <div class="duration-row">
          <div class="duration-field">
            <input type="number" id="edit-dur-hours" min="0" max="12" placeholder="0" />
            <span class="duration-unit">hr</span>
          </div>
          <div class="duration-field">
            <input type="number" id="edit-dur-mins" min="0" max="59" placeholder="0" />
            <span class="duration-unit">min</span>
          </div>
          <span class="end-time-preview" id="edit-end-preview"></span>
        </div>
        <span class="learning-points-hint">Adjust hours/minutes — end time updates automatically.</span>
        <input type="hidden" id="edit-end" name="end_time" />
      </div>
      ...
      <div class="form-group">
        ...
        <textarea id="edit-learning-points" name="learning_points" rows="4"
          placeholder="Write each learning point on a new line:"></textarea>
        <span class="learning-points-hint"><i class="fa-solid fa-lightbulb" style="margin-right:3px;"></i>Each line = one bullet point.</span>
      </div>
      ...
      <div class="form-group">
        <label>Workshop Image <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
        <p style="font-size:0.8rem;color:#64748b;margin-bottom:8px;margin-top:0;"><i class="fa-solid fa-circle-info" style="color:#2c7be5;margin-right:4px;"></i>Paste a direct image link.</p>
        <input type="url" id="edit-image" name="image_path" class="img-url-input" placeholder="e.g. https://images.unsplash.com/photo-..." />
        <small class="img-help-text"><i class="fa-solid fa-lightbulb" style="margin-right:3px;"></i>Leave blank to keep the existing image.</small>
      </div>
  ...
  <div class="admin-modal admin-modal-small">
    <div class="admin-modal-header">
      <div class="admin-modal-title-group">
        ...
        <div><p class="admin-modal-kicker">Confirm Action</p><h2>Delete Workshop</h2></div>
      </div>
    ...
    <div class="admin-delete-body">
      ...
      <p class="admin-delete-warning"><i class="fa-solid fa-triangle-exclamation"></i> This cannot be undone. All student bookings for this workshop will also be removed.</p>
    </div>
    ...
    <div class="admin-modal-actions">
      ...
      <button type="button" class="btn btn-danger" id="confirm-delete-btn"><i class="fa-solid fa-trash"></i> Yes, Delete</button>
    </div>
  </div>
</div>
...
async function resolveFeedback(id, btn) {
  if (!confirm('Mark this feedback as resolved? It will be hidden from this view.')) return;
  ...
  if (result.success) {
    ...
    showAdminToast('Feedback resolved.', 'success');
  } else { alert(result.message || 'Could not resolve.'); }
}
...
async function saveEditedMessage(feedbackId, messageId, btn) {
  ...
  } else { alert(result.message || 'Could not update message.'); }
}
...
<script src="../../scripts/admin.js?v=10"></script>
...
// ── FEEDBACK BADGE: disappears instantly when admin scrolls to feedback ──
(function() {
