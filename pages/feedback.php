...
// Converts a database timestamp into a UTC ISO value for browser-side formatting.
function feedback_time_to_utc_iso(?string $timestamp): string
{
    if (empty($timestamp)) {
        return '';
    }
          ...
          <div class="feedback-reply-notification">
            <div class="frn-header">
              ...
              <div>
                ...
                <span
                  class="js-local-time"
                  data-utc-time="<?= h(feedback_time_to_utc_iso($fb['submitted_at'] ?? '')) ?>"
                >
                  <?= h($fb['submitted_at'] ?? '') ?>
                </span>              </div>
            </div>
